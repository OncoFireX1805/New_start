...
source("C:/Users/parth/OneDrive/Desktop/New_start/multi_QCR.R")
...
DatasheetB
multiqc_data<-read.csv("C:/Users/parth/OneDrive/Desktop/New_start/multiqc_table_1.csv")
multiqc_data
View(multiqc_data)
...
```{r}
sampleInfo<-merge(sampleInfo,multiqc_data,by="sample",all.x=TRUE)
```
...
```{r}
library(ggplot2)
results_annotated<-as.data.frame(results_annotated[1:10, ])

ggplot(data = results_annotated)+geom_are(mapping =aes(x=SYMBOL,y=padj))
```
